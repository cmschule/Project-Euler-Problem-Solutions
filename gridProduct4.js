...
for (var i = 0; i < 20; i++) {
	...
	for (var j = 0; j < 60; j+=3) {
		...
		gridNum[i].push(Number(grid[60*i+j]+grid[60*i+j+1]));
	}
}
var max = 0;
var product = 0;
...
for (var r = 0; r < 20; r++) {
	for (var c = 0; c < 16; c++) {
		//across
		product = gridNum[r][c]*gridNum[r][c+1]*gridNum[r][c+2]*gridNum[r][c+3];
		if (product>max) {
			max = product;
		}
		//down
		product = gridNum[c][r]*gridNum[c+1][r]*gridNum[c+2][r]*gridNum[c+3][r];
		if (product>max) {
			max = product;
		}
	}
